const materia = {
    ...
    methods: {
        ...
        validarFormulario() {
            if (!this.codigo.trim()) {
                alertify.error('El código de la materia no puede estar vacío.');
                return false;
            }
            if (!this.nombre.trim() || this.nombre.length < 3) {
                alertify.error('El nombre de la materia debe tener al menos 3 caracteres.');
                return false;
            }
            if (!this.uv.trim()) {
                alertify.error('Las unidades valorativas no pueden estar vacías.');
                return false;
            }
            return true;
        },
